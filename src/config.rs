...
fn value(name: &str, file: &HashMap<String, String>) -> Option<String> {
    env::var(name).ok().or_else(|| file.get(name).cloned()).filter(|s| !s.is_empty())
}
...
impl Config {
    pub fn load() -> Self {
        ...
        Self {
            ...
            db_path: PathBuf::from(value("ECONOMICS_DB", &file).unwrap_or_else(|| "runtime/economics.db".into())),
    ...
    pub fn print_key_status(&self) {
        ...
        println!("KRX_API_URL: {}", if self.krx_api_url.is_some() { "configured" } else { "missing" });
    }
}
