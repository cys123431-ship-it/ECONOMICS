...
| 수집 | FRED 28계열, ALFRED 빈티지 날짜 페이지네이션·100개 단위 최초발표 조회·일시 오류 재시도·계열별 재수집, Treasury 개별 CUSIP+일별 집계, Binance 7개 파생계열, ECOS 페이지네이션·재시도·계열별 재수집, 승인된 KRX 31개 공식 고정 엔드포인트·서비스별 집계·최근 7일 증분 갱신·API ID별 재수집을 구현한다. | 실제 FRED·ALFRED·ECOS·Treasury·Binance·KRX 스모크 수집, KRX 31개 ID·경로·저장 계열 고유성 및 저장 건수 확인 |
